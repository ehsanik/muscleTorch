...
class MoCoGazeModel(BaseModel):
    metric = []

    def __init__(self, args):
        super(MoCoGazeModel, self).__init__(args)
    ...
    def forward(self, input, target):
        ...
        output, (hidden, cell) = self.lstm(spatial_features) #To see the whole sequence and embed it into a hidden vector

        output, (hidden, cell) = self.decoder_lstm(spatial_features, (hidden, cell))

        predicted_gaze = self.gaze_unembed(output)
